...
const createCustomer = async function (req, res) {
    try {
        ...
        if (!validator.validInput(firstName)) return res.status(400).send({ status: false, message: 'firstName should be valid string' })
        if (!validator.validName(firstName)) return res.status(400).send({ status: false, message: 'firstName should be valid' })

        if (!validator.validInput(lastName)) return res.status(400).send({ status: false, message: 'lastName should be valid string' })
        if (!validator.validName(lastName)) return res.status(400).send({ status: false, message: 'lastName should be valid' })

        if (!validator.validInput(emailID)) return res.status(400).send({ status: false, message: 'Email should be valid string' })
        if (!validator.validEmail(emailID)) return res.status(400).send({ status: false, message: 'Email should be valid' })

        if (!validator.validInput(mobileNumber)) return res.status(400).send({ status: false, message: 'mobileNumber should be valid string' })
        ...
        if (!validator.validInput(address)) return res.status(400).send({ status: false, message: 'Address should be valid string' })
